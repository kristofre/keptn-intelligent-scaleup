...
type IntelligentScaleupEventData struct {
	keptnv2.EventData
}

// ServiceName specifies the current services name (e.g., used as source when sending CloudEvents)
const ServiceName = "keptn-intelligent-scaleup"
const IntelligentScaleupEvent = "keptn-intelligent-scaleup"
...
func processKeptnCloudEvent(ctx context.Context, event cloudevents.Event) error {
	...
	**/
	...
	case keptnv2.GetTriggeredEventType(IntelligentScaleupEvent): // sh.keptn.event.your-event.triggered
		log.Printf("Processing keptn-intelligent-scaleup.triggered Event")

		eventData := &keptnv2.ActionTriggeredEventData{}
		parseKeptnCloudEventPayload(event, eventData)

		log.Printf("EVENTDATA: %s", eventData.Problem.ProblemID)

		return HandleIntelligentScaleupEvent(myKeptn, event, eventData)
...
func _main(args []string, env envConfig) int {
	...
	log.Println("Starting keptn-intelligent-scaleup...")
